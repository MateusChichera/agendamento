        ...
        usuarioModel.autenticar(email, senha)
            .then(user => {
                if (user) {
                    ...
                    res.cookie('userId', user.usuid, { httpOnly: false, secure: false, maxAge: 4 * 60 * 60 * 1000 });
                    res.cookie('userHash', userHash, { httpOnly: false, secure: false, maxAge: 4 * 60 * 60 * 1000 });
                    res.cookie('Nome', user.usunome, { httpOnly: false, secure: false, maxAge: 4 * 60 * 60 * 1000 });
